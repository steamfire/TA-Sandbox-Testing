...
Chapter 1 - Standalone Starch routines
...
Check spraying it on:  [there is another section in the Starch world section that does more Check spraying it on]
	if the noun is starch and the player holds the starch:
		if the second noun is the dog:
...
chapter 1 - General world improvements
...
Chapter 2 - Starch Spraying

A room can be slippery.  A room has a number called slips. The slips of a room is usually 0. 
...
Short spraying is an action applying to one thing.  [Understand when the player only says SPRAY X]
